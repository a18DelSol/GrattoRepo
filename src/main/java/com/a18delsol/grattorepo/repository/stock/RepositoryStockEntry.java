...
public interface RepositoryStockEntry extends CrudRepository<ModelStockEntry, Integer> {
    Optional<ModelStockEntry> findByIDAndEntityDeleteFalse(Integer ID);
    Iterable<ModelStockEntry> findByEntityDeleteFalse();

    @Query(nativeQuery=true,
    value="SELECT * FROM model_stock_entry"
    + " WHERE (entity_delete is false)"
    + " AND (:entryCountMin is null OR entry_count >= :entryCountMin)"
